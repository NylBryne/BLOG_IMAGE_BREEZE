...
<x-slot name="header">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('posts.create') }}" class="bg-black hover:bg-blue-600 text-white px-4 py-2 rounded-md">ADD</a>
    </div>
</x-slot>
    ...
    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400 flex items-center">
        <a href="{{ route('posts.show', $post->id) }}" class="border border-blue-500 bg-blue-200 hover:bg-blue-500 text-blue-700 hover:text-white px-4 py-2 rounded-md mr-2">SHOW</a>
        <a href="{{ route('posts.edit', $post->id) }}" class="border border-yellow-500 bg-yellow-200 hover:bg-yellow-500 text-yellow-700 hover:text-white px-4 py-2 rounded-md mr-2">EDIT</a>
        {{-- add delete button using form tag --}}
        <form method="post" action="{{ route('posts.destroy', $post->id) }}" class="inline">
            @csrf
            @method('delete')
            <button type="submit" class="border border-red-500 bg-red-200 hover:bg-red-500 text-red-700 hover:text-white px-4 py-2 rounded-md">DELETE</button>
        </form>
    </td>
</tr>
